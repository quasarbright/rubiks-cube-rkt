...
; Puzzle (listof FaceMove) -> Puzzle
(define (perform-face-moves pzl moves)
  (for/fold ([pzl pzl])
            ([move moves])
    (perform-face-move pzl move)))
...
(define (face-configuration-num-face-turns face-config)
  (if (zero? (length (face-configuration-face-colors face-config)))
      0
...
(define (scramble pzl [num-moves 100])
  (define possible-moves (puzzle-possible-moves pzl))
  (for/fold ([pzl pzl])
            ([_ (in-range num-moves)])
    (define move (random-choice possible-moves))
    (perform-face-move pzl move)))

; Puzzle -> (listof FaceMove)
; all possible moves on the puzzle. ignores moves that don't do anything.
; order doesn't matter. doesn't return duplicates
(define (puzzle-possible-moves pzl)
  ; TODO update once we have big puzzles
  (define face-config (puzzle-face-config pzl))
  (define face-colors (face-configuration-face-colors face-config))
  (for*/list ([face-color face-colors]
              [num-turns (in-range 1 (face-configuration-num-face-turns face-config))])
    (face-move face-color num-turns)))

(module+ test
  (test-case "puzzle-possible-moves 3x3 cube"
    (define moves (puzzle-possible-moves (solved-3x3-cube)))
    (check-equal? (length moves) (* 3 6))
    (check-not-false (member (face-move WHITE 2) moves))))

; (listof A) -> A
; random element from list. list must be non-empty
(define (random-choice lst)
  (when (zero? (length lst))
    (error 'random-choice "cannot choose from empty list"))
  (list-ref lst (random (length lst))))

; solves a 3x3 cube
#;(define (solve/3x3-cfop pzl)
  (define pzl/cross (solve/3x3-cfop/cross pzl))
  (define pzl/f2l (solve/3x3-cfop/f2l pzl/cross))
...
; find the shortest sequence of moves that leaves the puzzle solved according to the predicate.
; Returns #f if it can't find a solution
(define (solve/search pzl solved? [max-moves 20])
  (let/ec return
    ; bfs search
    ; assume possible moves is static
    (define possible-moves (puzzle-possible-moves pzl))
    ; each element of the queue is a (list Puzzle (listof FaceMove))
    ; where the move list is the reversed list of moves that led to that puzzle
    (define queue (list (list pzl (list))))
    ...
    (let loop ()
      (unless (empty? queue)
        ...
        (when (solved? pzl)
          (return (list pzl (reverse moves-rev))))
        (when (< (length moves-rev) max-moves)
          (push*! (for/list ([move possible-moves])
                    (list (perform-face-move pzl move)
                          (cons move moves-rev))))
          (loop))))
    #f))

(module+ test
  (test-case "solve/search"
    (define pzl (perform-face-moves (solved-3x3-cube) (list (face-move WHITE 1) (face-move RED 1))))
    (define result (solve/search pzl puzzle-solved?))
    (check-equal? result (list (solved-3x3-cube) (list (face-move RED 3) (face-move WHITE 3))))))
